...
class QTable:
    ...
    def best_actions(self, state_hash):
        ...
        best_acts = [act for act, q_value in self.q_table[state_hash].items(
    ...
    def load(cls, filename, initial_value=0.0):
        ...
        instance.q_table = defaultdict(lambda: defaultdict(
            lambda: initial_value), loaded_q_table)
        return instance
...
class UCB(ExplorationStrategy):
    ...
    def select_action(self, agent, obs):
        ...
        action_combinations = [list(x) for x in np.ndindex(
            *agent.env.action_space.nvec)]

        for action in action_combinations:
            q_value = agent.q_table.get_q_value(obs, action)
            state_count = agent.state_visits.get(
                obs, 1)  # Avoid division by zero
            state_action_key = (obs, tuple(action))
            state_action_count = agent.state_action_visits.get(
                state_action_key, 1)
            ucb_value = q_value + self.c * \
                np.sqrt(np.log(state_count) / state_action_count)
...
class QLearning(BaseAgent):
    ...
    def learn(self, obs_hash, actions, next_state_hash, rewards, done):
        ...
        target = rewards + self.discount_factor * \
            max_next_q_value if not done else rewards
        current_q_value = self.q_table.get_q_value(obs_hash, actions)
        updated_value = current_q_value + \
            self.learning_rate * (target - current_q_value)
...
def game_loop(env, agent, training=False, num_episodes=1, msx_steps_per_episode=300, render=False, create_video=False, qtable_file=None):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            agent.state_visits[obs_hash] = agent.state_visits.get(
                obs_hash, 0) + 1

            # Update state-action visit count
            state_action_key = (obs_hash, tuple(actions))
            agent.state_action_visits[state_action_key] = agent.state_action_visits.get(
                state_action_key, 0) + 1
...
if __name__ == "__main__":
    ...
    # Train the agent
    game_loop(env, agent, training=True, num_episodes=100,
              msx_steps_per_episode=300, render=False, qtable_file='qtable')
